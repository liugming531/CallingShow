// TestProtocol.cpp : 定义控制台应用程序的入口点。
//
// EvhttpTest.cpp : 定义控制台应用程序的入口点。
//
/* Base on code from:
...
static char *content = "<?xml version=\"1.0\" encoding =\"UTF-8\" standalone =\"yes\"?>"
"<request>"
"<reqHeader>"
"<token></token>"
"<appid>netviom</appid>"
...
int start_url_request(struct http_request_get *http_req, int req_get_flag)
{
	...
	*/
	...
	if (req_get_flag == REQUEST_POST_FLAG) {
		...
		evhttp_add_header(http_req_post->req->output_headers, "User-Agent", "PC");
		evhttp_add_header(http_req_post->req->output_headers, "Accept", http_req_post->content_type);
	}
	else if (req_get_flag == REQUEST_GET_FLAG) {
		...
		if (len > 1) {
			...
			sprintf_s(path_query,len, "%s?%s", path, query);
		}
...
int main(int argc, char *argv[])
{
	...
	struct http_request_post *http_req_post = (http_request_post*)start_http_requset(base,
		"http://58.249.48.74:8080/services/imsall/v1/login.api",
		REQUEST_POST_FLAG,
		"application/xml",
		content);
